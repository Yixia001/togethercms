(function ($) {
  ...
  Drupal.friendly_register.checkUserName = function (userName) {
    $.getJSON(userName.ajaxPath + encodeURIComponent(userName.oldValue), function(data) {
      if (!data.flood) {
        ...
        $('.phoneNoErrInfo').after('<div id="edit-name-check" class="' + cssclass + '"><span class="text">' + message + '</span></div>');
  ...
  Drupal.behaviors.friendly_register = {
    attach: function (context, settings) {
      ...
      userName.avail = Drupal.t('号码没有注册过.');
      userName.notAvail = Drupal.t('号码已经注册过.');
