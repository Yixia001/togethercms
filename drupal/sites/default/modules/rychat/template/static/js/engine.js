...
(function ($) {
    ...
    //是否开启声音
        ...
        $scope = {};
    var conversationStr = '<li targetType="{0}" targetId="{1}" targetName="{2}"><span class="user_img"><img src={3} onerror="this.src=\'' + basepath + 'static/images/personPhoto.png\'"/><font class="conversation_msg_num {4}">{5}</font></span><span class="conversationInfo"><p style="margin-top: 10px"><font class="user_name">{6}</font><font class="date" >{7}</font></p></span></li>';
    var historyStr = '<div class="xiaoxiti {0} user"><div class="user_img"><img onerror="this.src=\'' + basepath + 'static/images/personPhoto.png\'" src="{1}"/></div><span>{2}</span><div class="msg"><div class="msgArrow"><img src="' + basepath + 'static/images/{3}"> </div><span></span>{4}</div><div messageId="{5}" class="status"></div></div><div class="slice"></div>';
    var friendListStr = '<li targetType="4" targetId="{0}" targetName="{1}"><span class="user_img"><img src={2} onerror="this.src=\'' + basepath + 'static/images/personPhoto.png\'"/></span> <span class="user_name">{1}</span></li>';
    var discussionStr = '<li targetId="{0}" targetName="{1}" targetType="{2}"><span class="user_img"><img src="' + basepath + 'static/images/user.png"/></span><span class="user_name">{3}</span></li>';
  ...
  //登陆人员信息默认值
    owner = {
            id: "",
            portrait: basepath + "static/images/user_img.jpg",
            name: "张亚涛"
        },
    friend = {
        id: ""
...
//初始化登陆人员信息
    ...
    $.ajax({
        ...
        if (data.code == 200) {
            ...
            $('span[RCTarget="owner.name"]').html(token.username);
            //链接融云
    ...
    function addhistoryMessages(item) {
        ...
        $(".dialog_box:first").append(String.stringFormat(historyStr, item.getMessageDirection() == 0 ? "other_user" : "self", item.getMessageDirection() == 1 ? owner.portrait : basepath + "static/images/personPhoto.png", "", item.getMessageDirection() == 0 ? 'white_arrow.png' : 'blue_arrow.png', myUtil.msgType(item), item.getMessageId()));
    }

    function initConversationList() {
        _html = "";
        $scope.ConversationList.forEach(function (item) {
            _html += String.stringFormat(conversationStr, item.getConversationType().valueOf(), item.getTargetId(), item.getConversationTitle(), basepath + "static/images/personPhoto.png", item.getUnreadMessageCount() == 0 ? "hidden" : "", item.getUnreadMessageCount(), item.getConversationTitle(), new Date(+item.getLatestTime()).toString().split(" ")[4]);
        });
    ...
    function getHistory(id, name, type) {
        ...
        $scope.historyMessages.forEach(function (item, i) {
            _html += String.stringFormat(historyStr, item.getMessageDirection() == 0 ? "other_user" : "self", item.getMessageDirection() == 1 ? owner.portrait : basepath + "static/images/personPhoto.png", "", item.getMessageDirection() == 0 ? 'white_arrow.png' : 'blue_arrow.png', myUtil.msgType(item), item.getMessageId());
        });
